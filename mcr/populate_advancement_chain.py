...
def create_adv_item(entry: Union[LootTableEntry, ItemEntry], _: dict[str, RandomizerTemplate]) -> AdvItem:
...
def populate_advancement_chain(root_name: str, rand_templates: dict[str, RandomizerTemplate]):
    root_map: RandomizerTemplate = rand_templates[root_name]
    build_chain: bool = True
    entries: list[ItemEntry | LootTableEntry]
    current_map: RandomizerTemplate = root_map

    chain_info = AdvChainInfo(
        last_link = create_and_fix_name(root_name, eAdvItemType.root, rand_templates),
    ...
    while build_chain:
        ...
        from_items_link = create_and_fix_name(current_map.target_name, eAdvItemType.root, rand_templates)
        ...
        for entry in entries:
            handle_adv_item(entry, current_map, root_map, chain_info, rand_templates)

        current_map = rand_templates[chain_info.next_name]
...
def handle_adv_item(entry: Union[ItemEntry, LootTableEntry], current_map: RandomizerTemplate, root_map: RandomizerTemplate, chain_info: AdvChainInfo, rand_templates: dict[str, RandomizerTemplate]):
    adv_item = create_adv_item(entry, rand_templates)
    ...
    if not chain_info.found_link and adv_item.name == current_map.target_name and adv_item.adv_item_type is eAdvItemType.item and rand_templates[adv_item.name].original.type_ is eLootTable.block:
        chain_info.last_link = adv_item
        adv_item.adv_item_type = eAdvItemType.block
        fix_name(adv_item, rand_templates[adv_item.name])
        chain_info.next_name = current_map.target_name
        chain_info.found_link = True
    else:
        if current_map.name not in root_map.adv_branches:
